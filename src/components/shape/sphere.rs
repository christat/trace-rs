...
mod tests {
    ...
    fn intersect_sphere() {
      ...
      let sphere_shape = Sphere::default();
      let shape = &Shape::Sphere(sphere_shape);
      ...
      assert_eq!(res, sphere_shape.intersect(r, position, shape, transform, material));
      ...
      assert_eq!(None, sphere_shape.intersect(r, position, shape, transform, material));
    ...
    fn sphere_normal_at() {
      ...
      let normal = s.normal_at(&point, &position, &transform);
      ...
      let transform = Transform(
        Matrix4::scaling(1.0, 0.5, 1.0) * Matrix4::rotation_z(std::f32::consts::PI / 5.0),
      );
